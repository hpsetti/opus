                ...
                >
                  <div class="">
                    <div class="pl-40 flex" :class="!isPurchased && !['Software', 'Hardware & On-Device Markings'].includes(selected.sub_category_name) ? 'disable-filters' : 'not-disable-filters'">
                      ...
                      <p class="pl-20 sub-category-name">{{ selected.sub_category_name }}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="col-xs-12 mt-20 table-wrapper">
          <div class="table">
            <div class="recommendation-intro d-flex justify-content-between">
              <p class="root-heading">Recommendation</p>
              <p
                class="text-subtitle-1 blue-3 float-right"
                :class="[
                  rootCauseInfo.length > 0 ? 'cursor-pointer' : 'no-action'
                ]"
              ...
              >
                ...
                >
                  <div class="panel-heading">
                    <div
                      class="panel-heading recommentation-arrow-container p-relative pb-0"
                      aria-expanded="false"
                      data-toggle="collapse"
                    ...
                    >
                      <p
                        class="text-button-label blue-1 pr-20"
                        :class="
                      ...
                      <div
                        class="recommendation-before-collapse d-flex w-90"
                        v-if="!closeRecommendationId[root.id]"
                      >
                        <div class="panel-body panel-basis-40 margin-nve-10">
                          <p
                            class="text-regular-large black-2 drl-custom-ul"
                            v-if="root.recommendation_content.length > 250"
                          >
                            <span
                              v-html="
                                root.recommendation_content.substr(0, 90) +
                                '...'
                              "
                            ></span>
                          </p>
                          <p
                            ...
                            v-else
                          >
                            <span v-html="root.recommendation_content"></span>
                          </p>
                        </div>
                        <div class="panel-basis-60 margin-nve-10">
                          <img
                            class="image-croped"
                            :src="getImageUrl(root.recommendation_image_path)"
                          />
                        </div>
                      </div>
                      <div
                      ...
                      >
                        <div class="panel-body panel-basis-40 margin-nve-10">
                          <div class="text-regular-large black-2 drl-custom-ul">
                            <p v-html="root.recommendation_content"></p>
                          </div>
                        </div>
                        <div class="panel-basis-60 margin-nve-10">
                          <img
            ...
            >
              ...
              <h3 class="select-filter-text text-color">
                ...
                view matching design recommendations
              </h3>
          ...
          >
            ...
            <market-place-tools
              :loader="loaderStatus"
              :userId="userId"
              toolName="Design Recommendation Library"
            ></market-place-tools>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    isAllExpanded() {
      if (!Object.keys(this.closeRecommendationId)) {
        return false;
      }
      return Object.values(this.closeRecommendationId).every((item) => item);
    }
  },
  async created() {
    await Promise.all([this.getdrlFilters(), this.getList()]);
  ...
  methods: {
    ...
    async searchRootCause(query) {
      ...
      const data = await drlRecommendationBySearch(formData);
    ...
    async getdrlFilters() {
      const data = await getdrlFilters();
    ...
    async getDRLData(filterList) {
      ...
      const marketoForm = {
        ...
        page_urlreferral_extended: document.referrer,
        form_control: 'Tools',
        form_control_details: 'Design Recommendation Library'
      };
      postMarketoFormData(marketoForm);
      this.loaderStatus = false;
    },
