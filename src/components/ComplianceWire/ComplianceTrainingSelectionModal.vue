        ...
        <div class="modal-body">
          <div class="text-h5 blue-1">
            <img
              ...
              alt=""
            />
            <span>{{ steps[step].title }}</span></div>
          <div class="text-regular-small black-5 mb-20 card-left-spacing">
            <span>{{ steps[step].desc }}</span>
          </div>
          <div class="d-flex justify-content-center col-gap-35" v-if="step === 'type'">
            <CourseSelectionCard type="live" :price="399" has-shadow :selected="selectedOption === 'live'" @selected="setSelected"></CourseSelectionCard>
            <CourseSelectionCard type="prerec" :price="199" has-shadow :selected="selectedOption === 'prerec'" @selected="setSelected"></CourseSelectionCard>
            <CourseSelectionCard type="bundle" :price="499" has-shadow :selected="selectedOption === 'bundle'" @selected="setSelected"></CourseSelectionCard>
          </div>
          ...
          <div v-else class="card-left-spacing">
            <div class="course-review mb-15">
              <div v-for="item in Object.keys(review)" :key="item" class="row m-0 mb-20">
                <div class="text-button-label black-3 col-xs-3 p-0">{{ `${item}:` }}</div>
                <div class="text-button-label blue-1 col-xs-9 p-0" v-html="review[item]"></div>
              </div>
            </div>
            <div class="text-body-light black-2" v-if="steps[step].footer" v-html="steps[step].footer[selectedOption]">
            </div>
          </div>
        </div>
        <hr />
        <div class="footer col-xs-12 p-0 flex-wrapper">
          <BaseButton @click="updateStep(true)" variant="secondary" v-if="steps[step].back" class="margin-right-auto">
            Back
          </BaseButton>
          ...
          <BaseButton class="br-20 ml-20" :disabled="!isNextEnabled" @click="updateStep(false)">{{ steps[this.step].btnTitle }}</BaseButton>
        </div>
...
import { getTimeSlotByCourseIdForUser, updateTimeSlot } from '../../apis/instructorLedTraining';
import BaseButton from '../UIComponents/BaseButton.vue';
import EventBus from '../../utils/event-bus';
import CourseSelectionCard from './ComplianceCourseSelectionCard.vue';
...
export default {
      ...
      steps: {
        ...
        review: {
          ...
          footer: {
            ...
            prerec: 'Note: Purchasing this course will allow you to watch the full recording until 60 days.',
            bundle: 'If any issues or scheduling conflicts arise, feel free to contact our team anytime. You can also change the date and time of your course after purchasing by visiting the course details page. <br> Note: Purchasing this course will allow you to watch the full recording until 60 days.',
          },
  ...
  methods: {
    ...
    async getTimeSlotsList() {
      ...
      const availableApiTimeSlots = await getTimeSlotByCourseIdForUser(this.courseDetails.course_id);
    ...
    populateReview() {
      const slot = this.availableTimeSlots.find((s) => s.slot_id === this.selectedSlotId);
      if (this.selectedOption === 'prerec') {
        ...
        this.review['Course method'] = 'Bundle: Instructor-Led Training and Pre-recorded Course';
        this.review.Date = this.getSlotDate(slot);
        this.review.Time = this.getSlotTimeRange(slot);
        this.review.Location = 'Online - Microsoft Teams Meeting <br> Video file link - OPUS Trainings Library';
