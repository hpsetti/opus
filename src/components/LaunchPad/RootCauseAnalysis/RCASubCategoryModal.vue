...
export default {
  ...
  async created() {
    this.emitter.on('getAllCategoryDataEmit', () => {
  ...
  methods: {
    ...
    async addCategory() {
      ...
      if (this.actionSelected === 'Add') {
        ...
        if (response) {
          if (response.data === 'Subcategory name already exist') {
            this.$notify({
              type: 'warning',
              title: 'Warning',
              text: response.data
        ...
        if (response) {
          if (response.data === 'SubCategoryName Already exist') {
            this.$notify({
              type: 'warning',
              title: 'Warning',
              text: response.data
  ...
  // mounted() {
  //   $('#addSubCategoryModal').on('shown.bs.modal', async () => {
  //     this.loaderStatus = true;
  //     const response = await getAllSubCategoryData();
  //     this.allCategoryList = response.filter((item) => item.active === true);
  //     this.loaderStatus = false;
