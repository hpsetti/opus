...
export async function getDesignLibraryData(formData) {
  let data;
  await axios
    .post(`${import.meta.env.VITE_VUE_APP_JAVA_API_URL}/uirl/uirlRequirement`, formData)
    .then((response) => {
...
export async function getuirlFilters() {
  let data;
  await axios
    .get(`${import.meta.env.VITE_VUE_APP_JAVA_API_URL}/uirl/filters`)
    .then((response) => {
      if (response.status === 200 && response.data) {
...
export async function getPopularSearch() {
  let result = [];
  await axios
    .get(`${import.meta.env.VITE_VUE_APP_JAVA_API_URL}/uirl/uirlSubCategory`)
    .then((response) => {
      if (response && response.data.status_code === 200) {
        result = response.data.data;
      }
